...
module.exports = function BoundaryEvent(activityElement, parentContext) {
  ...
  function AttachedToListener(attachedToApi, attachedToExecution) {
    ...
    function getActivateEventDefinitions() {
      if (activeDefinitions && (activeDefinitions.length > 0 && activeDefinitions[0] != undefined)) return activeDefinitions;
      activeDefinitions = getEventDefinitions().map((ed) => {
        let acti = getActivityExecution();
        let ev = eventDefinitionEmitter(ed);
        let ss = ed.activate(activityApi, acti, ev);
        return ss;
      });
    ...
    function resumeEventDefinitions(state) {
      ...
      activeDefinitions = getEventDefinitions().map((ed) => {
        ed.resume(state, activityApi, getActivityExecution(), eventDefinitionEmitter(ed))
      });
      return activeDefinitions;
    }
