...
pub.getActivityIO = function(moddleContext, activityId) {
  ...
  if (element) {
    element.inputParameters = element.$children.filter((v) => v.$type === 'camunda:inputParameter');
    element.outputParameters = element.$children.filter((v) => v.$type === 'camunda:outputParameter');
  }
  return element;
};
