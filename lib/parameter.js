...
function Parameter(ioParm) {
  ...
  function getMap(message, variablesAndServices) {
    ...
    return entries.reduce((result, entry) => {
      // allow map keys to be evaluted as templates
      let name = entry.name;
      if ( expressions.hasExpression(name) ) {
        script = scriptHelper.parse(`${name}.io`, "`" + name + "`");
        name = scriptHelper.execute(script, variablesAndServices);
      }
      result[name] = entry.getInputValue(message, variablesAndServices);
      return result;
    }, {});
  }
